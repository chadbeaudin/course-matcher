...
- **Road/trail network**: self-hosted Overpass API instance (`overpass.bigtimber.cloud`, on the user's unRAID box) for OSM data — same instance StreetSweep uses. Reuse the layered-cache fetch pattern from `StreetSweep/lib/overpass.ts` (in-memory → Postgres `osm_cache` via Prisma → network, tile-grid snapped) rather than rebuilding it.
