...
async function pollTask(taskUrl: string, apiKey: string, accessToken: string): Promise<RwgpsTask> {
  ...
  while (Date.now() - start < 30000) {
    ...
    const task: RwgpsTask = data.task ?? data;
...
/** Uploads a GPX course to the user's RideWithGPS library via their OAuth access token. */
export async function uploadRouteToRwgps(
  accessToken: string,
  gpxContent: string,
  name: string
  ...
  if (!uploadRes.ok) {
    ...
    throw new Error(
      `RideWithGPS upload failed: ${uploadRes.status} ${uploadRes.statusText} - ${errorText.substring(0, 300)}`
    );
  }
