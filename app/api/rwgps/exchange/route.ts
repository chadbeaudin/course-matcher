...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!clientId || !clientSecret || !code) {
      return NextResponse.json({ error: 'Missing required parameters or server configuration' }, { status: 400 });
    }
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'RideWithGPS OAuth failed', details: data }, { status: response.status });
    }

    return NextResponse.json({ access_token: data.access_token });
  } catch (err) {
    const message = err instanceof Error ? err.message : 'RideWithGPS OAuth exchange failed';
